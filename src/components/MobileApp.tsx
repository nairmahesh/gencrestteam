...
const MobileApp: React.FC = () => {
  ...
  const {
    overallMetrics: liquidationMetrics,
    distributors: liquidationDistributors,
    loadingMetrics,
    loadingDistributors,
    fetchOverallMetrics,
    fetchDistributors
  ...
  const canBypassLocation = ['ADMIN', 'RMM', 'RBH', 'ZBH', 'VP', 'CFO', 'CHRO', 'MH', 'MD'].includes(currentUserRole.toUpperCase());

  // Fetch liquidation data when tab becomes active
  useEffect(() => {
    if (activeTab === 'liquidation') {
      fetchOverallMetrics('distributor');
      fetchDistributors(1, 10);
    }
  }, [activeTab]);
  ...
  const isWithinGeofence = (): { valid: boolean; distance: number; message: string; bypassed?: boolean } => {
    // Bypass location check for admin roles
    if (canBypassLocation) {
      return {
        valid: true,
        distance: 0,
        message: 'Location check bypassed for your role.',
        bypassed: true
      };
    }
  ...
  const handleStartDay = async () => {
    // Admin roles can start day without location
    if (!canBypassLocation && (!latitude || !longitude)) {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          start_latitude: canBypassLocation ? 0 : latitude,
          start_longitude: canBypassLocation ? 0 : longitude
        })
        .select()
        .single();
      ...
      const message = canBypassLocation
        ? `Day started at ${now.toLocaleTimeString()} (Location check bypassed for ${currentUserRole})`
        : `Day started at ${now.toLocaleTimeString()}`;
      alert(message);
    } catch (error: any) {
      console.error('Error starting day:', error);
      alert(error.message || 'Error starting day');
    }
  };

  const handleEndDay = async () => {
    // Admin roles can end day without location
    if (!canBypassLocation && (!latitude || !longitude)) {
    ...
    if (!attendanceId || !startTime) {
      alert('No active day session found');
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          end_latitude: canBypassLocation ? 0 : latitude,
          end_longitude: canBypassLocation ? 0 : longitude,
      ...
      const message = canBypassLocation
        ? `Day ended at ${now.toLocaleTimeString()} (Location check bypassed for ${currentUserRole})\nTotal hours: ${totalHours.toFixed(2)}`
        : `Day ended at ${now.toLocaleTimeString()}\nTotal hours: ${totalHours.toFixed(2)}`;
      alert(message);
      setDayStarted(false);
      setStartTime(null);
      ...
      alert(error.message || 'Error ending day');
    }
  };
  ...
  // Using centralized mock data - easily replaceable with API calls
  // See src/data/mockData.ts for the complete data structure and API replacement guide
  const distributors = liquidationDistributors.length > 0 ? liquidationDistributors : MOCK_DISTRIBUTORS.length > 0 ? MOCK_DISTRIBUTORS : [
    {
      id: 'DIST001',
  ...
  // Liquidation filtered data - moved to top level to avoid hooks violation
  const liquidationFilteredData = useMemo(() => {
  ...
  // Liquidation pagination - moved to top level
  const liquidationPagination = usePagination({
    data: liquidationFilteredData,
    itemsPerPage: 5,
    initialPage: 1
  });
...
`;
  ...
  const renderLiquidationContent = () => {
    ...
    <div className="flex-1 p-3 space-y-3 overflow-y-auto">
        ...
        <div className="bg-orange-50 rounded-lg p-3 border-l-4 border-orange-500">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-orange-600 mb-1">
              {loadingMetrics ? '...' : formatCurrency(metrics.openingStock.value)}
            </div>
        ...
        <div className="bg-blue-50 rounded-lg p-3 border-l-4 border-blue-500">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-blue-600 mb-1">
              {loadingMetrics ? '...' : formatCurrency(metrics.ytdNetSales.value)}
            </div>
        ...
        <div className="bg-green-50 rounded-lg p-3 border-l-4 border-green-500">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-green-600 mb-1">
              {loadingMetrics ? '...' : formatCurrency(metrics.liquidation.value)}
            </div>
        ...
        <div className="bg-purple-50 rounded-lg p-3 border-l-4 border-purple-500">
          <div className="text-center">
            ...
            <div className="text-2xl font-bold text-purple-600 mb-1">
              {loadingMetrics ? '...' : formatCurrency(metrics.balanceStock.value)}
            </div>
      ...
      <div className="bg-white rounded-xl p-3 border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors flex items-center gap-1.5 shadow-sm"
            >
              <Package className="w-3.5 h-3.5" />
              <span>Batch Update</span>
            </button>
            <span className="text-xs text-gray-600 font-medium bg-gray-100 px-2 py-1 rounded-md">
              {liquidationFilteredData.length} {searchType}s
            </span>
          </div>
        </div>

        {/* Search Controls - Matching Web Design */}
        <div className="mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
          <div className="flex gap-2 mb-2">
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium bg-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
            >
            ...
            <input
              ...
              placeholder={`Search by ${searchType} name or code...`}
              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
            />
          </div>
          <button
            onClick={() => {}}
            ...
            <div key={distributor.id} className="border border-gray-200 rounded-lg p-3">
              ...
              <div className="grid grid-cols-4 gap-2 mb-3">
                <div className="bg-orange-50 rounded p-2 text-center border border-orange-200">
                  ...
                  <div className="text-lg font-bold text-orange-800 my-1">{formatCurrency(distributor.metrics?.openingStock?.value || distributor.openingStock?.value || 0)}</div>
                  <button
                    onClick={() => {
                ...
                <div className="bg-blue-50 rounded p-2 text-center border border-blue-200">
                  ...
                  <div className="text-lg font-bold text-blue-800 my-1">{formatCurrency(distributor.metrics?.ytdNetSales?.value || distributor.ytdNetSales?.value || 0)}</div>
                  <button
                    onClick={() => {
                      setSelectedDistributorForMetric(distributor);
                ...
                <div className="bg-green-50 rounded p-2 text-center border border-green-200">
                  ...
                  <div className="text-lg font-bold text-green-800 my-1">{formatCurrency(distributor.metrics?.liquidation?.value || distributor.liquidation?.value || 0)}</div>
                  <button
                    onClick={() => {
                      setSelectedDistributorForMetric(distributor);
                ...
                <div className="bg-purple-50 rounded p-2 text-center border border-purple-200">
                  ...
                  <div className="text-lg font-bold text-purple-800 my-1">{formatCurrency(distributor.metrics?.balanceStock?.value || distributor.balanceStock?.value || 0)}</div>
                  <button
                    onClick={() => {
                      setSelectedDistributorForMetric(distributor);
              ...
              <div className="mb-3">
                <div className="flex justify-between text-xs text-gray-600 mb-1">
                  ...
                  <span className="font-semibold">{(distributor.metrics?.liquidationPercentage || distributor.liquidationPercentage || 0).toFixed(2)}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-500 ${
                      (distributor.metrics?.liquidationPercentage || distributor.liquidationPercentage || 0) >= 50
                        ? 'bg-green-500'
                        : (distributor.metrics?.liquidationPercentage || distributor.liquidationPercentage || 0) >= 30
                        ? 'bg-yellow-500'
                        : 'bg-red-500'
                    }`}
                    style={{ width: `${distributor.metrics?.liquidationPercentage || distributor.liquidationPercentage || 0}%` }}
                  ></div>
                </div>
              </div>
        ...
        {/* Pagination */}
        {!loadingDistributors && liquidationFilteredData.length > 0 && (
          <div className="mt-4">
            <Pagination
              currentPage={currentPage}
              totalPages={totalPages}
              totalItems={liquidationFilteredData.length}
              itemsPerPage={5}
  ...
  const renderActivityTrackerContent = () => {
    const allActivities = [
      { id: 1, type: 'Visit', title: 'Distributor Visit - Sharma Distributors', status: 'completed', date: '2024-01-25', time: '10:30 AM', location: 'Delhi', icon: MapPin, color: 'blue' },
      { id: 2, type: 'Stock Update', title: 'Stock Verification - DIST001', status: 'completed', date: '2024-01-25', time: '11:45 AM', location: 'Delhi', icon: Package, color: 'green' },
      { id: 3, type: 'Meeting', title: 'Monthly Review Meeting', status: 'pending', date: '2024-01-26', time: '02:00 PM', location: 'Head Office', icon: Users, color: 'purple' },
      { id: 4, type: 'Order', title: 'New Order - RET045', status: 'pending', date: '2024-01-26', time: '03:30 PM', location: 'Delhi', icon: ShoppingCart, color: 'orange' },
      { id: 5, type: 'Report', title: 'Weekly Sales Report', status: 'completed', date: '2024-01-24', time: '05:00 PM', location: 'Online', icon: FileText, color: 'teal' },
      { id: 6, type: 'Task', title: 'Follow up with retailers', status: 'pending', date: '2024-01-27', time: '10:00 AM', location: 'Delhi', icon: CheckSquare, color: 'pink' }
    ];

    const filteredActivities = activityFilter === 'all'
      ? allActivities
      : allActivities.filter(a => a.status === activityFilter);
      ...
      <div className="flex-1 p-3 space-y-3 overflow-y-auto">
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-3 text-white">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-1 rounded-lg text-xs font-medium transition-colors"
            >
              View Full
            </button>
          </div>
          <p className="text-sm opacity-90">Track all your field activities</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            className={`py-2 px-3 rounded-lg text-sm font-medium transition-colors ${
              activityFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-3 rounded-lg text-sm font-medium transition-colors ${
              activityFilter === 'pending' ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
          ...
          <button
            ...
            className={`py-2 px-3 rounded-lg text-sm font-medium transition-colors ${
              activityFilter === 'completed' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
            Done ({allActivities.filter(a => a.status === 'completed').length})
          </button>
        </div>
        ...
        <div className="space-y-3">
          {filteredActivities.map((activity) => {
            ...
            return (
              <div key={activity.id} className="bg-white rounded-xl p-3 border border-gray-200">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2">
                      <span className={`inline-block px-2 py-1 rounded text-xs font-medium bg-${activity.color}-50 text-${activity.color}-700`}>
                        {activity.type}
                      </span>
                    </div>
                  </div>
        ...
        {filteredActivities.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>No {activityFilter} activities found</p>
          </div>
        )}
      </div>
    );
  ...
  const renderDocsContent = () => (
    <div className="flex-1 p-3 space-y-3 overflow-y-auto">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-6 text-white">
        ...
        <p className="text-sm opacity-90">Complete developer guide with APIs, endpoints, and implementation details</p>
      </div>

      {/* Quick Links */}
      <div className="grid grid-cols-2 gap-3">
        <a
          href="/docs/MOBILE_APP_DEVELOPER_GUIDE.md"
          target="_blank"
          className="bg-white border-2 border-blue-500 rounded-lg p-3 hover:bg-blue-50 transition-colors"
        >
          <div className="flex flex-col items-center text-center">
            <Code className="w-8 h-8 text-blue-600 mb-2" />
            <span className="font-semibold text-gray-900 text-sm">Full Guide</span>
            <span className="text-xs text-gray-600 mt-1">Complete documentation</span>
          </div>
        </a>
        <a
          href="/docs/business-logic.md"
          target="_blank"
          className="bg-white border-2 border-green-500 rounded-lg p-3 hover:bg-green-50 transition-colors"
        >
          <div className="flex flex-col items-center text-center">
            <Zap className="w-8 h-8 text-green-600 mb-2" />
            <span className="font-semibold text-gray-900 text-sm">Business Logic</span>
            <span className="text-xs text-gray-600 mt-1">Rules & workflows</span>
          </div>
        </a>
      </div>
        ...
        <div className="bg-white rounded-xl p-3 border border-gray-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="bg-gray-50 rounded p-3">
              <p className="font-mono text-xs text-blue-600 mb-1">GET /distributors</p>
              <p className="text-gray-600">Fetch all distributors with filters</p>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <p className="font-mono text-xs text-blue-600 mb-1">GET /retailer_inventory</p>
              <p className="text-gray-600">Get SKU-level stock data</p>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <p className="font-mono text-xs text-blue-600 mb-1">POST /stock_transactions</p>
              <p className="text-gray-600">Record stock updates</p>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <p className="font-mono text-xs text-blue-600 mb-1">POST /verifications</p>
              <p className="text-gray-600">Submit verification proof</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-3 border border-gray-200">
          ...
          <div className="space-y-2">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <p className="font-semibold text-sm text-gray-900">Liquidation Management</p>
                <p className="text-xs text-gray-600">SKU-wise tracking with invoice details</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Stock Update Flow</p>
                <p className="text-xs text-gray-600">Three-tab modal with verification</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Geolocation & Geofencing</p>
                <p className="text-xs text-gray-600">Real-time location validation</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Entity 360° View</p>
                <p className="text-xs text-gray-600">Complete distributor/retailer info</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Activity Tracker</p>
                <p className="text-xs text-gray-600">Track visits, orders, meetings & tasks</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Batch Stock Updates</p>
                <p className="text-xs text-gray-600">Update multiple SKUs at once</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-green-600 mt-0.5" />
              <div>
                <p className="font-semibold text-sm text-gray-900">Visit Management</p>
                <p className="text-xs text-gray-600">Check-in/out with photo capture</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-3 border border-gray-200">
          ...
          <div className="space-y-3 text-xs">
            <div className="bg-purple-50 rounded p-3 border border-purple-200">
              <p className="font-mono font-bold text-purple-900 mb-2">Distributor Model</p>
              <div className="space-y-1 text-gray-700">
                <p>• id, name, code, businessType</p>
                <p>• openingStock, ytdNetSales</p>
                <p>• liquidation, balanceStock</p>
                <p>• territory, region, zone</p>
              </div>
            </div>
            <div className="bg-blue-50 rounded p-3 border border-blue-200">
              <p className="font-mono font-bold text-blue-900 mb-2">SKU Model</p>
              <div className="space-y-1 text-gray-700">
                <p>• code, name, unit</p>
                <p>• openingStock, currentStock</p>
                <p>• transactions[]</p>
              </div>
            </div>
            <div className="bg-green-50 rounded p-3 border border-green-200">
              <p className="font-mono font-bold text-green-900 mb-2">Transaction Model</p>
              <div className="space-y-1 text-gray-700">
                <p>• invoiceDate, type (Sale/Return)</p>
                <p>• quantity, value</p>
                <p>• productCode, skuCode</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-3 border border-gray-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="bg-red-50 rounded p-3 border border-red-200">
              <p className="font-semibold text-red-900 mb-1">Configuration</p>
              <p className="text-xs text-gray-700">Radius: 1000m (1km)</p>
              <p className="text-xs text-gray-700">Algorithm: Haversine Formula</p>
            </div>
            <div className="bg-orange-50 rounded p-3 border border-orange-200">
              <p className="font-semibold text-orange-900 mb-1">Validation Points</p>
              <p className="text-xs text-gray-700">• Stock updates blocked outside geofence</p>
              <p className="text-xs text-gray-700">• Verification requires location proof</p>
              <p className="text-xs text-gray-700">• Visit check-in/out tracked</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-3 border border-gray-200">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-blue-50 rounded p-2">
              <p className="font-semibold text-blue-900">React 18.3+</p>
              <p className="text-gray-600">with TypeScript</p>
            </div>
            <div className="bg-cyan-50 rounded p-2">
              <p className="font-semibold text-cyan-900">Tailwind CSS</p>
              <p className="text-gray-600">Styling</p>
            </div>
            <div className="bg-green-50 rounded p-2">
              <p className="font-semibold text-green-900">Supabase</p>
              <p className="text-gray-600">Backend + Auth</p>
            </div>
            <div className="bg-purple-50 rounded p-2">
              <p className="font-semibold text-purple-900">Lucide React</p>
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-xl p-3 text-white">
        <p className="font-bold mb-2">📚 Need More Details?</p>
        <p className="text-sm opacity-90 mb-3">
          Access the complete developer guide with code examples, best practices, and troubleshooting tips.
        </p>
        <a
          href="/docs/MOBILE_APP_DEVELOPER_GUIDE.md"
          target="_blank"
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'activity':
        return renderActivityTrackerContent();
      case 'tasks':
      ...
      <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-3 text-white">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h2 className="font-bold text-sm">Sandeep</h2>
              <p className="text-xs opacity-90">Kumar</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs font-medium opacity-90">{currentUserRole}</p>
            <p className="text-[10px] opacity-75">Delhi Region</p>
          </div>
        </div>
        
        {/* Monthly Plan Section - Compact */}
        <div className="bg-white bg-opacity-20 rounded-lg p-2 mb-2 border border-white border-opacity-30">
          <button
            ...
            className="w-full flex items-center justify-between hover:bg-white hover:bg-opacity-10 transition-colors rounded-lg px-1"
          >
            <div className="flex items-center space-x-2">
              <div className="w-7 h-7 bg-white bg-opacity-30 rounded-lg flex items-center justify-center">
                <Calendar className="w-3.5 h-3.5 text-white" />
              </div>
              <div className="text-left">
                <h3 className="text-sm font-semibold text-white">Work Plan Assignment</h3>
            ...
            <div className="flex items-center space-x-1">
              ...
              <ChevronDown className={`w-3.5 h-3.5 text-white transition-transform ${showWorkPlan ? 'rotate-180' : ''}`} />
            </div>
          </button>
          ...
          {showWorkPlan && (
            <div className="mt-4 pt-4 border-t border-white border-opacity-30 space-y-3">
              ...
              <button
                ...
                className="w-full bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg p-3 border border-white border-opacity-30 transition-colors flex items-center justify-center space-x-2 text-white font-medium text-sm"
              >
                <ExternalLink className="w-4 h-4" />
                <span>View Full Work Plan</span>
              </button>

              {/* Monthly Activity Summary */}
              <div className="bg-white bg-opacity-20 rounded-lg p-3 border border-white border-opacity-30">
                <h4 className="font-semibold text-white text-sm mb-3">Monthly Activity Summary</h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-white bg-opacity-20 rounded p-2">
                    <div className="text-white opacity-75">Planned</div>
                    <div className="text-white font-bold text-lg">45</div>
                  </div>
                  <div className="bg-white bg-opacity-20 rounded p-2">
                    <div className="text-white opacity-75">Done</div>
                    <div className="text-white font-bold text-lg">38</div>
                  </div>
                  <div className="bg-white bg-opacity-20 rounded p-2">
                    <div className="text-white opacity-75">% Pending</div>
                    <div className="text-yellow-200 font-bold text-lg">15.6%</div>
                  </div>
                  <div className="bg-white bg-opacity-20 rounded p-2">
                    <div className="text-white opacity-75">% Completed</div>
                    <div className="text-green-200 font-bold text-lg">84.4%</div>
                  </div>
                </div>
              </div>
              ...
              {/* Activity Breakdown */}
              <div className="space-y-2">
                <h4 className="font-semibold text-white text-sm">Activity Breakdown</h4>

                {/* MDO Activities */}
                <div className="bg-white bg-opacity-20 rounded-lg p-3">
                  <div className="text-white font-medium text-xs mb-2 flex items-center justify-between">
                    <span>MDO Activity</span>
                    <span className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs">28/32</span>
                  </div>
                  <div className="space-y-1 text-xs text-white opacity-90">
                    <div className="flex justify-between">
                      <span>• Farmer Meeting - Small</span>
                      <span className="font-medium">8/10</span>
                    </div>
                    <div className="flex justify-between">
                      <span>• Farmer Meeting - Large</span>
                      <span className="font-medium">4/5</span>
                    </div>
                    <div className="flex justify-between">
                      <span>• Demo - Organised</span>
                      <span className="font-medium">5/6</span>
                    </div>
                    <div className="flex justify-between">
                      <span>• Demo - Spot Demo</span>
                      <span className="font-medium">3/3</span>
                    </div>
                    <div className="flex justify-between">
                      <span>• Jeep Campaign</span>
                    ...
                    <div className="flex justify-between">
                      <span>• Individual Farmer Connect</span>
                      <span className="font-medium">2/2</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white bg-opacity-20 rounded-lg p-3">
                  <div className="text-white font-medium text-xs mb-2 flex items-center justify-between">
                    ...
                    <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded-full text-xs">10/13</span>
                  </div>
                  <div className="space-y-1 text-xs text-white opacity-90">
                    <div className="flex justify-between">
                      <span>• Farmer Meeting - Small</span>
                    ...
                    <div className="flex justify-between">
                      <span>• Distributor Meeting</span>
                      <span className="font-medium">1/1</span>
                    </div>
                    <div className="flex justify-between">
                      <span>• Retailer Meeting</span>
        ...
        {/* Attendance Tracking & Notifications - Compact Row */}
        <div className="flex items-center justify-between space-x-2 mb-2">
          {!dayStarted ? (
            <button
              onClick={handleStartDay}
              className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 px-3 rounded-lg font-semibold flex items-center justify-center space-x-2 transition-colors shadow-md text-sm"
            >
            ...
            <>
              <div className="flex-1 bg-green-500 text-white py-2 px-3 rounded-lg font-semibold flex items-center justify-center space-x-2 text-sm">
                <Clock className="w-4 h-4 animate-pulse" />
                <span>Active: {startTime?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
              <button
                onClick={handleEndDay}
                className="bg-red-500 hover:bg-red-600 text-white py-2 px-3 rounded-lg font-semibold flex items-center justify-center transition-colors shadow-md text-sm"
              >
          ...
          {/* Notification Icons - Inline */}
          <div className="flex space-x-1.5">
            <button
              onClick={() => alert('Tasks: 4 pending')}
              className="w-7 h-7 bg-yellow-500 rounded-full flex items-center justify-center hover:bg-yellow-600 transition-colors relative"
            >
            ...
            <button
              ...
              className="w-7 h-7 bg-red-500 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors relative"
            >
              <AlertTriangle className="w-3.5 h-3.5 text-white" />
              <span className="absolute -top-0.5 -right-0.5 bg-yellow-500 text-white text-[10px] rounded-full w-3.5 h-3.5 flex items-center justify-center font-bold">
                3
              </span>
            </button>
            <button
              onClick={() => alert('Messages: 5 unread')}
              className="w-7 h-7 bg-blue-500 rounded-full flex items-center justify-center hover:bg-blue-600 transition-colors relative"
            >
              <Mail className="w-3.5 h-3.5 text-white" />
              <span className="absolute -top-0.5 -right-0.5 bg-green-500 text-white text-[10px] rounded-full w-3.5 h-3.5 flex items-center justify-center font-bold">
                5
              </span>
            </button>
            <button
              onClick={() => alert('Approvals: 2 pending')}
              className="w-7 h-7 bg-green-500 rounded-full flex items-center justify-center hover:bg-green-600 transition-colors relative"
            >
              <User className="w-3.5 h-3.5 text-white" />
              <span className="absolute -top-0.5 -right-0.5 bg-purple-500 text-white text-[10px] rounded-full w-3.5 h-3.5 flex items-center justify-center font-bold">
                2
              </span>
            </button>
          </div>
        ...
        <div className="flex justify-around items-center px-2 py-2">
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[60px] transition-all ${
              activeTab === 'home' ? 'text-orange-600' : 'text-gray-500'
            }`}
          >
            <div className={`p-2 rounded-2xl transition-all ${
              activeTab === 'home' ? 'bg-orange-50' : ''
            }`}>
              <Home className={`transition-all ${
                activeTab === 'home' ? 'w-6 h-6' : 'w-5 h-5'
              }`} />
            </div>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[60px] transition-all ${
              activeTab === 'activity' ? 'text-orange-600' : 'text-gray-500'
            }`}
          >
            <div className={`p-2 rounded-2xl transition-all ${
              activeTab === 'activity' ? 'bg-orange-50' : ''
            }`}>
              <Activity className={`transition-all ${
                activeTab === 'activity' ? 'w-6 h-6' : 'w-5 h-5'
              }`} />
            </div>
            <span className={`text-xs mt-1 font-medium ${
              activeTab === 'activity' ? 'text-orange-600' : 'text-gray-500'
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[60px] transition-all ${
              activeTab === 'liquidation' ? 'text-orange-600' : 'text-gray-500'
            }`}
          >
            <div className={`p-2 rounded-2xl transition-all ${
              activeTab === 'liquidation' ? 'bg-orange-50' : ''
            }`}>
              <Droplets className={`transition-all ${
                activeTab === 'liquidation' ? 'w-6 h-6' : 'w-5 h-5'
              }`} />
            </div>
            <span className={`text-xs mt-1 font-medium ${
              activeTab === 'liquidation' ? 'text-orange-600' : 'text-gray-500'
          ...
          >
            <div className={`p-2 rounded-2xl transition-all ${
              activeTab === 'reports' ? 'bg-orange-50' : ''
            }`}>
              <FileText className={`transition-all ${
                activeTab === 'reports' ? 'w-6 h-6' : 'w-5 h-5'
              }`} />
            </div>
            <span className={`text-xs mt-1 font-medium ${
              activeTab === 'reports' ? 'text-orange-600' : 'text-gray-500'
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[60px] transition-all ${
              activeTab === 'orders' ? 'text-orange-600' : 'text-gray-500'
            }`}
          >
            <div className={`p-2 rounded-2xl transition-all ${
              activeTab === 'orders' ? 'bg-orange-50' : ''
            }`}>
              <ShoppingCart className={`transition-all ${
                activeTab === 'orders' ? 'w-6 h-6' : 'w-5 h-5'
              }`} />
            </div>
            <span className={`text-xs mt-1 font-medium ${
              activeTab === 'orders' ? 'text-orange-600' : 'text-gray-500'
              ...
              <div className="bg-white rounded-xl p-3 border border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-blue-50 rounded-lg p-3 text-center border border-blue-200">
                    ...
                    <div className="text-lg font-bold text-blue-800">₹{(selected360Distributor.creditLimit / 100000).toFixed(2)}</div>
                    <div className="text-xs text-blue-500">Rs. Lakhs</div>
                  </div>
                  ...
                  <div className="bg-green-50 rounded-lg p-3 text-center border border-green-200">
                    ...
                    <div className="text-lg font-bold text-green-800">₹{(selected360Distributor.totalPurchases / 100000).toFixed(2)}</div>
                    <div className="text-xs text-green-500">Rs. Lakhs</div>
                  </div>
                  ...
                  <div className="bg-purple-50 rounded-lg p-3 text-center border border-purple-200">
                    ...
                    <div className="text-lg font-bold text-purple-800">₹{(selected360Distributor.balanceCreditLimit / 100000).toFixed(2)}</div>
                    <div className="text-xs text-purple-500">Rs. Lakhs</div>
                  </div>
                  ...
                  <div className="bg-orange-50 rounded-lg p-3 text-center border border-orange-200">
                    ...
                    <div className="text-lg font-bold text-orange-800">₹{(selected360Distributor.totalPayments / 100000).toFixed(2)}</div>
                    <div className="text-xs text-orange-500">Rs. Lakhs</div>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[9999] flex items-center justify-center p-0">
          <div className="bg-white w-full max-w-[375px] h-full max-h-screen overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {verifyTab === 'stock-details' && (
                <>
                  <h4 className="text-sm font-semibold text-gray-900 mb-3">Product & SKU Breakdown</h4>

                  {/* Simple Flat Table - Mobile Optimized */}
                      ...
                      <div className="overflow-hidden border border-gray-200 rounded-lg shadow-sm">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              <th scope="col" className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase">
                                Product
                              </th>
                              <th scope="col" className="px-2 py-2 text-left text-xs font-semibold text-gray-700 uppercase">
                                SKU
                              </th>
                              <th scope="col" className="px-2 py-2 text-center text-xs font-semibold text-gray-700 uppercase">
                                Current
                              </th>
                              <th scope="col" className="px-2 py-2 text-center text-xs font-semibold text-gray-700 uppercase">
                                Update
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            <tr className="bg-white">
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">Samta NPK 6-0-18</div>
                                <div className="text-xs text-gray-500">fgsamf00050</div>
                              </td>
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">1 Ltr</div>
                                <div className="text-xs text-gray-500">Ltr</div>
                              </td>
                              <td className="px-2 py-3 text-center">
                                <div className="text-xs font-semibold text-gray-900">10 Kg/Ltr</div>
                                <div className="text-xs text-gray-500">₹7.00K</div>
                              </td>
                              <td className="px-2 py-3">
                                <input
                                  type="number"
                                  placeholder="10"
                                  className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:ring-2 focus:ring-orange-500 focus:border-transparent text-center"
                                />
                              </td>
                            </tr>
                            ...
                            <tr className="bg-gray-50">
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">Agropurna AP Gr.6</div>
                                <div className="text-xs text-gray-500">fgcmgm0127</div>
                              </td>
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">1 Ltr</div>
                                <div className="text-xs text-gray-500">Ltr</div>
                              ...
                              <td className="px-2 py-3 text-center">
                                <div className="text-xs font-semibold text-gray-900">10 Kg/Ltr</div>
                                <div className="text-xs text-gray-500">₹6.30K</div>
                              </td>
                              <td className="px-2 py-3">
                                <input
                                  type="number"
                            ...
                            <tr className="bg-white">
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">SAMTA NPK (11:11:8)</div>
                                <div className="text-xs text-gray-500">fgmvag0020</div>
                              </td>
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">500 ml</div>
                                <div className="text-xs text-gray-500">ml</div>
                              </td>
                              <td className="px-2 py-3 text-center">
                                <div className="text-xs font-semibold text-gray-900">10 Kg/Ltr</div>
                                <div className="text-xs text-gray-500">₹8.50K</div>
                              </td>
                              <td className="px-2 py-3">
                                <input
                                  type="number"
                            ...
                            <tr className="bg-gray-50">
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">SAMTA NPK (8:8:8)</div>
                                <div className="text-xs text-gray-500">fgmvag0025</div>
                              </td>
                              <td className="px-2 py-3">
                                <div className="text-xs font-medium text-gray-900">1 Ltr</div>
                                <div className="text-xs text-gray-500">Ltr</div>
                  ...
                  {/* Footer Info */}
                  <div className="mt-3 text-xs text-gray-600 bg-orange-50 border border-orange-200 rounded-lg p-2.5">
                    <strong className="text-orange-900">4 SKU(s)</strong> ready for verification
                  </div>
                </>
              )}
                      ...
                      onClick={() => {
                        ...
                        if (!geofenceCheck.valid) {
                          setLocationModalData({
                            ...
                            locationName: OUTLET_LOCATION.name
                          });
...
Date/Time: ${new Date().toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}`}
      ...
      {show360View && selected360Distributor && (
        <Entity360View
          distributorCode={selected360Distributor?.code || selected360Distributor}
      ...
      <LocationVerificationModal
        ...
        locationName={locationModalData?.locationName || OUTLET_LOCATION.name}
        requiredRadius={GEOFENCE_RADIUS}
      />
    </div>
  );
};
