...
const TSMDashboard: React.FC = () => {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'my-meetings' | 'team-activities' | 'approvals'>('overview');
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      {/* Territory Mapping Cards - Clickable */}
      <div className="grid grid-cols-3 gap-4">
        <button
          onClick={() => setShowMappingView(showMappingView === 'mdo' ? 'hidden' : 'mdo')}
          className={`bg-white rounded-xl p-6 card-shadow card-hover text-center transition-all ${
            showMappingView === 'mdo' ? 'ring-2 ring-purple-500' : ''
          }`}
        >
          <div className="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <User className="w-6 h-6 text-purple-600" />
          </div>
          <div className="text-3xl font-bold text-purple-600 mb-1">{mappingData.mdos.length}</div>
          <div className="text-sm text-gray-600 font-medium">MDOs</div>
          <div className="text-xs text-gray-500 mt-1">Click to view mapping</div>
        </button>

        <button
          onClick={() => setShowMappingView(showMappingView === 'distributor' ? 'hidden' : 'distributor')}
          className={`bg-white rounded-xl p-6 card-shadow card-hover text-center transition-all ${
            showMappingView === 'distributor' ? 'ring-2 ring-blue-500' : ''
          }`}
        >
          <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <Building className="w-6 h-6 text-blue-600" />
          </div>
          <div className="text-3xl font-bold text-blue-600 mb-1">{mappingData.totalDistributors}</div>
          <div className="text-sm text-gray-600 font-medium">Distributors</div>
          <div className="text-xs text-gray-500 mt-1">Click to view mapping</div>
        </button>

        <button
          onClick={() => setShowMappingView(showMappingView === 'retailer' ? 'hidden' : 'retailer')}
          className={`bg-white rounded-xl p-6 card-shadow card-hover text-center transition-all ${
            showMappingView === 'retailer' ? 'ring-2 ring-green-500' : ''
          }`}
        >
          <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <Users className="w-6 h-6 text-green-600" />
          </div>
          <div className="text-3xl font-bold text-green-600 mb-1">{mappingData.totalRetailers}</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Navigation Tabs */}
        <div className="bg-white rounded-xl p-2 card-shadow mb-8">
          <div className="flex space-x-2">
            {[
              { id: 'overview', label: 'Overview', icon: BarChart3 },
              { id: 'my-meetings', label: 'My Meetings', icon: Calendar },
              { id: 'team-activities', label: 'Team Activities', icon: Users },
        ...
        {selectedView === 'my-meetings' && renderMyMeetings()}
        {selectedView === 'team-activities' && renderTeamActivities()}
