/*
  # Retailer Stock Flow System

  ## Overview
  Creates tables for tracking stock flow: Distributor → Retailer → Farmer
  Enables MDOs to verify retailer stock and record sales to farmers or returns.

  ## 1. New Tables

  ### `stock_transfers`
  - Tracks all stock movements between entities
  - Records distributor-to-retailer and retailer-to-farmer transfers
  - Includes GPS coordinates and audit trail

  ### `retailer_inventory`
  - Maintains current stock levels for each retailer by SKU
  - Tracks total received from distributors and total sold to farmers
  - Auto-updates based on stock transfers
...
CREATE POLICY "Anon can read stock transfers"
  ON stock_transfers FOR SELECT TO anon USING (true);

CREATE POLICY "Anon can insert stock transfers"
  ON stock_transfers FOR INSERT TO anon WITH CHECK (true);

CREATE POLICY "Anon can read retailer inventory"
  ON retailer_inventory FOR SELECT TO anon USING (true);

CREATE POLICY "Anon can insert retailer inventory"
  ON retailer_inventory FOR INSERT TO anon WITH CHECK (true);

CREATE POLICY "Anon can update retailer inventory"
  ON retailer_inventory FOR UPDATE TO anon USING (true) WITH CHECK (true);
